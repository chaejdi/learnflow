...
- **시간표 주 단위 occurrence + 색상 + 시각화 개편** (마이그레이션 `007_schedule_weekly.sql`)
  ...
  - **수업별 색상 선택**(8색 팔레트, `CLASS_COLORS` 풀 클래스 고정). 과목별 기본색 시드(초등 emerald/중등기본 blue/심화 violet/고등 amber)
...
- 남은 1순위: 로그인 E2E·상담저장은 브라우저 직접 확인 필요(아래 TODO)
...
- [x] ~~짱짱맨 학원 시드(시간표/샘플 대화)로 대시보드·전환율 화면 채우기~~ → **2026-06-09 완료**: 정규 시간표 9건(4과목 주간), 과거 2~5월 백필(문의 89·예약 13)로 전환율 8.3%→25% 우상향 추세. `seedhist_*`/`notes='[seedhist]'` 마커로 멱등(재실행 가능). ※ 화면 눈으로 점검만 남음
